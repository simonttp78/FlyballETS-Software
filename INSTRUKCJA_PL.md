...
1. Uruchomić ETSa i odczytać wyświetlaną w trakcie uruchamiania wersję firmware (np. "ver. 1.14.0"). Od wersji 1.9.0 informacja o wersji firmware widoczna jest także w stopce interfejsu www (WiFi). Przy aktualizacji wersję od v.1.14.0 do v1.18.0 wymagają, aby komputer był podłączony kabelkiem Ethernet do routera, gdyż jest wymagane połączenie z Internetem. Alternatynie aktualizację należy wykonać smartfonem, gdyż po połączeniu przez WiFi z ETSem będzie miał on połączenie z Internetem przez sieć komórkową.
...
1. Uruchomić ETSa
2. Połączyć komputer z siecią WiFi nadawaną przez ETSa (domyślna nazwa sieci zaczyna się od "FlyballETS", domyślne hasło "FlyballETS.1234")
